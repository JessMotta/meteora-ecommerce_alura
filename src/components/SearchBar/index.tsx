...
const notMenu:string = 'bg-white border-black placeholder-(--color-gray)';

export function SearchBar(props: SearchBarInterface) {
    return (
        <div className={`space-x-4 ${props.show ? "flex" : "hidden"}  `}>
            <input type="text" placeholder="Digite o produto" className= {`py-2 px-3  ${props.menuPosition ? menu : notMenu}`}></input>
            <Button color="transparent">{props.button}</Button>
        </div>
    )
}